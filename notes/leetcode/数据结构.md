...
注意：start <= dict[s[i]] 也就是要小于等于重复字符的上一个位置时才重新开始计数，否则就加上该字符。  
...
class Solution(object):
    def lowestCommonAncestor(self, root, p, q):
        ...
        else: # 一定要把返回 root 的情况放最后面，有多种可能
            return root
```
...
class Solution(object):
    def lowestCommonAncestor(self, root, p, q):
        ...
        if left and right: # 在左子树找到一个，右子树也找到一个
            return root 
        if left: # 在左子树找到 p,q ，右子树没找到为 None
            return left
        return right
```
...
要求空间复杂度 O(1)，对于 [1,2,3,4,5,6,7], k=3  
...
def containsNearbyAlmostDuplicate(self, nums: List[int], k: int, t: int) -> bool:
    n = len(nums)
    if t == 0 and n == len(set(nums)): # 投机了，因为最后的超大用例就是 t == 0
        return False
    for i in range(n):
...
要求时间复杂度为 O(n)， 空间复杂度 O(1), 并且不能使用除法。
...
[Leetcode : 160. Intersection of Two Linked Lists (Easy)](https://leetcode.com/problems/intersection-of-two-linked-lists/description/) 内容事业群，手机看的 工具开发
