...
## 二叉树
二叉树（Binary Tree）是 n 个结点的有限集合，该集合可以为空集（称为空二叉树），或者由一个根结点和两棵互不相交的、分别称为根结点的左子树和右子树的二叉树组成。  
...
思路：反转二叉树，对于一个结点 root 而言，只需要将其左孩子和右孩子交换即可实现反转，递归调用即可。
...
思路：利用递归实现，如果根结点为空，则直接返回 False，如果到最后都没有出现 sum == 0 的情况时，再进行一轮递归，则 root == None， 因此也返回 False。必须要注意的是最后的返回一定是递归的结果，当 sum == 0 并且该结点为叶子结点时才返回 True。
...
思路： 由于输入只有 root，而判断对称树需要比较左右结点是否相等，因此可以构造一个辅助函数 symmetric 来比较左右结点的值。  
1. 终止条件是已经到了叶子结点，即 left == None and right == None  
2. 当左右结点的值相等时，继续递归比较左结点的左子树与右结点的右子树是否相等，以及左结点的右子树与右结点的左子树是否相等。
...
思路：平衡二叉树是二叉树的任意结点的两颗子树之间的高度差小于等于 1。对于平衡二叉树，其左子树和右子树也是平衡二叉树，因此可以递归判断。需要构造一个函数来求二叉树的高度，可以用之前的 maxDepth。  
