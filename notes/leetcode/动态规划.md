...
<div align="center"><img src="http://latex.codecogs.com/gif.latex?dp[i][j][k]=max(dp[i-1][j][k],dp[i-1][j-weights1[i]][k-weights2[i]]&plus;values[i])"></div>  
<br>
优化空间，使用二维形式表示：  
<div align="center"><img src="dp[j][k]=max(dp[j][k],dp[j-weights1[i]][k-weights2[i]]+values[i])"></div>
