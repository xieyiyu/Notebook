...
* [背包问题](#背包问题)
    ...
    * [背包问题练习](#背包问题练习)
        ...
        * [得到目标和](#得到目标和)
        * [按单词列表分割字符串](#按单词列表分割字符串)
        * [用0-1组成最多的字符串](#用0-1组成最多的字符串)
        * [找零钱](#找零钱)
        * [组合总和](#组合总和)
* [分割整数](#分割整数)
    * [完全平方数](#完全平方数)
...
状态转移公式为：  
<div align="center"><img src="http://latex.codecogs.com/gif.latex?dp[j]=min(dp[j]&plus;dp[j-coins[i]&plus;1])"></div>
...
思路：用 dp[i] 表示 target 为 i 时，nums[] 可以组合的总数，那么对于上述的 dp[7]，遍历一遍 nums[], dp[4] = dp[3] + dp[2] + dp[1] （3，2，1三个数字），因此状态转移公式为：
...
备注： 1.这里 dp[0] = 1 是因为当正好 nums[] 中有数字能组成 i 时，比如 3 一个数字就可以组成 dp[3]，那么 dp[3] += dp[0]，有一种解法  
